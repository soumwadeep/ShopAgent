...
import { ArrowRight, Camera, ExternalLink, History, Home, LoaderCircle, MapPin, Search, Settings2, ShoppingBag, Sparkles, X } from "lucide-react";
...
function AgentContent() {
  ...
  const runSearch = async (value = input, nextMarket = market) => {
    ...
    if (!user) return;
    setInput(text); setMarket(nextMarket); setError(""); setBusy(true); setResult(null);
    try {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto grid max-w-6xl gap-8 px-4 py-7 pb-24 sm:px-6 md:grid-cols-[minmax(0,1fr)_250px] md:py-12">
      <div className="min-w-0"><div className="mb-7"><p className="text-sm font-medium text-primary">Your shopping agent</p><h1 className="mt-1 text-3xl font-semibold sm:text-4xl">What are you looking for?</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Search official product feeds where connected. Prices and availability come from retailers; delivery depends on your exact address.</p></div>
        <form onSubmit={event => { event.preventDefault(); runSearch(); }} className="rounded-lg border bg-card p-3 shadow-sm"><div className="flex items-center gap-2"><Search className="ml-1 shrink-0 text-muted-foreground" size={20}/><input value={input} onChange={event => setInput(event.target.value)} placeholder="Product name or barcode" aria-label="Product name or barcode" className="min-w-0 flex-1 bg-transparent px-2 py-3 text-base outline-none"/><button type="button" onClick={() => setScanner(true)} title="Scan barcode" aria-label="Scan barcode" className="rounded-md p-2 text-muted-foreground hover:bg-muted"><Camera size={20}/></button><button disabled={busy} className="grid size-10 shrink-0 place-items-center rounded-md bg-primary text-primary-foreground disabled:opacity-50" aria-label="Search products">{busy ? <LoaderCircle size={19} className="animate-spin"/> : <ArrowRight size={20}/>}</button></div><div className="flex flex-wrap items-center gap-3 border-t px-1 pt-3"><label className="flex items-center gap-2 text-xs text-muted-foreground"><MapPin size={15}/><select value={market} onChange={event => setMarket(event.target.value as Market)} className="max-w-36 bg-transparent font-medium text-foreground outline-none" aria-label="Shopping market"><option value="IN">India</option><option value="US">United States</option><option value="GB">United Kingdom</option></select></label>{market === "IN" && <label className="flex items-center gap-2 text-xs text-muted-foreground">Delivery PIN<input value={pin} onChange={event => setPin(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" pattern="[0-9]{6}" maxLength={6} placeholder="6 digits" aria-label="Delivery PIN" className="w-24 rounded-md border bg-background px-2 py-1.5 text-foreground"/></label>}<span className="text-xs text-muted-foreground">Delivery confirmed at checkout</span></div></form>
